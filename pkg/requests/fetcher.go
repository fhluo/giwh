...
type Response struct {
	List   []GachaLog `json:"list"`         // 抽卡记录列表
...
// PrevPage 获取上一页抽卡记录
func (f *Fetcher) PrevPage() ([]GachaLog, error) {
	panic("not implemented")
}
...
// NextPage 获取下一页抽卡记录
func (f *Fetcher) NextPage() ([]GachaLog, error) {
	// 等待下次请求
	f.Wait()
...
// Next 获取下一条抽卡记录
func (f *Fetcher) Next() (GachaLog, error) {
	// 如果缓存为空，则获取下一页
	if len(f.Cache) == 0 {
		_, err := f.NextPage()
		if err != nil {
...
// All 获取所有抽卡记录
func (f *Fetcher) All() ([]GachaLog, error) {
	for {
		list, err := f.NextPage()
		if err != nil {
			return nil, err
...
func (f *Fetcher) Until(condition func(GachaLog) bool) ([]GachaLog, error) {
	for {
		// 如果已经完成，则退出
		if f.Done {
			break
		...
		// 如果满足条件，则退出
		if condition(log) {
			i := slices.IndexFunc(f.GachaLogs, func(l GachaLog) bool {
				return l.ID == log.ID
			})
...
func FetchAll(url RequestURL) ([]GachaLog, error) {
	var logs []GachaLog
...
func FetchAllUntil(url RequestURL, condition func(GachaLog) bool) ([]GachaLog, error) {
	var logs []GachaLog
